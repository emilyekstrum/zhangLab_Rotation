...
1. Loads the dataset from an `.rds` file
...
```{r seurat-workflow}
mouse_seurat_norm <- NormalizeData(mouse_seurat)
mouse_seurat_norm_VF <- FindVariableFeatures(mouse_seurat_norm, selection.method = "vst", nfeatures = 2000)
mouse_seurat_norm_VF_scale <- ScaleData(mouse_seurat_norm_VF, features = rownames(mouse_seurat_norm_VF))
mouse_seurat_PCA <- RunPCA(mouse_seurat_norm_VF_scale, features = VariableFeatures(mouse_seurat_norm_VF_scale))
