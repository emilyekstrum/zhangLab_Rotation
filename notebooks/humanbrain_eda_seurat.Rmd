...
params:
  ...
  # Optional but recommended (spatial coordinates; typically 2 columns like x/y or row/col)
  spatial_csv: "/Users/emilyekstrum/Desktop/zhangLab/data/DLPFC_Data/Sample151673/DLPFC_spatial_3639cells_2cols.csv"

  # Optional manual annotations (CSV preferred); leave "" to skip
  annotation_csv: "/Users/emilyekstrum/Desktop/zhangLab/data/DLPFC_Data/Sample151673/Manual_annotation_3639cells.csv"

  # Basic Seurat object filters
  assay_name: "RNA"
  min_cells: 3
  min_features: 200

  # Human mitochondrial prefix
  mt_prefix: "^MT-"

  # QC filtering (toggle + starting thresholds)
  filter: true
  nFeature_low: 200
  nFeature_high: 7500
  nCount_high: 60000
...
Reductions(seu)
ElbowPlot(seu, ndims = max(50, params$n_pcs))
```
...
message("Graphs present: ", paste(names(seu@graphs), collapse = ", "))

seu <- FindClusters(seu, resolution = params$clustering_resolution)
seu <- RunUMAP(seu, dims = pcs)

DimPlot(seu, reduction = "umap", label = TRUE) + NoLegend()
```
...
if (!is.null(xcol) && !is.null(ycol)) {
  df <- data.frame(seu@meta.data, cluster = Idents(seu))
...
```{r spatial-gene}
if (!is.null(xcol) && !is.null(ycol)) {
  gene <- "MBP"  # change to your marker of interest
  if (gene %in% rownames(seu)) {
    df <- data.frame(seu@meta.data, expr = FetchData(seu, vars = gene)[,1])
