...
# remove any all-zero spots (after subsetting)
spot_keep <- Matrix::colSums(mouse_exp_mat) > 0
mouse_exp_mat <- mouse_exp_mat[, spot_keep, drop = FALSE]
mouse_coords  <- mouse_coords[spot_keep, , drop = FALSE]

# remove near-constant genes
mouse_gene_vars <- Matrix::rowMeans(mouse_exp_mat^2) - (Matrix::rowMeans(mouse_exp_mat))^2
mouse_exp_mat <- mouse_exp_mat[mouse_gene_vars > 1e-8, , drop = FALSE]

# jitter duplicate coords if any
dup <- duplicated(mouse_coords)
if (any(dup)) {
  set.seed(1)
  mouse_coords[dup, ] <- mouse_coords[dup, ] + matrix(rnorm(sum(dup) * 2, sd = 1e-3), ncol = 2)
}
...
mouse_svgs <- nnSVG(input = mouse_exp_mat, spatial_coords = mouse_coords, BPPARAM = bp, verbose = TRUE)
```
...
# keep genes that are expressed in at least 10 spots
human_keep <- Matrix::rowSums(human_exp_mat > 0) >= 10
human_exp_mat <- human_exp_mat[human_keep, , drop = FALSE]

# remove any all-zero spots (after subsetting)
spot_keep <- Matrix::colSums(human_exp_mat) > 0
human_exp_mat <- human_exp_mat[, spot_keep, drop = FALSE]
human_coords  <- human_coords[spot_keep, , drop = FALSE]

# remove any all-zero genes (optional but recommended)
gene_keep0 <- Matrix::rowSums(human_exp_mat) > 0
human_exp_mat <- human_exp_mat[gene_keep0, , drop = FALSE]

# remove near-constant genes
human_gene_vars <- Matrix::rowMeans(human_exp_mat^2) - (Matrix::rowMeans(human_exp_mat))^2
human_exp_mat <- human_exp_mat[human_gene_vars > 1e-8, , drop = FALSE]

# jitter duplicate coords if any
dup <- duplicated(human_coords)
if (any(dup)) {
  set.seed(1)
  human_coords[dup, ] <- human_coords[dup, ] + matrix(rnorm(sum(dup) * 2, sd = 1e-3), ncol = 2)
}
...
```{r}
# check dimensions
dim(human_exp_mat) # genes by cells
dim(human_coords) # cells by 2
```
...
# run nnSVG to find spatially variable genes
param <- SerialParam(progressbar = TRUE)
human_svgs <- nnSVG(input = human_exp_mat, spatial_coords = human_coords, BPPARAM = bp, verbose = TRUE)
```
