...
params:
  mouse_seurat_SVG_path: "/Users/emilyekstrum/repos/zhangLab_Rotation/data/processed/svgs/seurat_mouse_svg_table.rds"
  mouse_nnSVG_SVG_path: "/Users/emilyekstrum/repos/zhangLab_Rotation/data/processed/svgs/nnSVG_mouse_results.rds"
  
  human_seurat_SVG_path: "/Users/emilyekstrum/repos/zhangLab_Rotation/data/processed/svgs/seurat_human_svg_table.rds"
  human_nnSVG_SVG_path: "/Users/emilyekstrum/repos/zhangLab_Rotation/data/processed/svgs/nnSVG_human_results.rds"
  seed: 123
---
...
length(mouse_common_genes) # 542 common genes
```
...
head(common_svg_genes, 20) # 265 overlapping genes in top 500
```
...
cor(nn_rank_vec, seu_rank_vec, method = "spearman", use = "complete.obs") # 0.731
```
...
length(human_common_genes) # 340 common genes
```
...
# overlapping genes
common_svg_genes <- intersect(nn_top, seu_top)
head(common_svg_genes, 20) # 178 overlapping genes in top 500
```
...
Low similarity between the two methods in identifying top spatially variable genes in the human data as indicated by the Jaccard index of 0.217. This agrees with the previous statement that nnSVG and Seurat FindSpatiallyVariableGenes() capture different aspects of spatial variability in gene expression.
...
cor(nn_rank_vec, seu_rank_vec, method = "spearman", use = "complete.obs") # 0.572
```
...
p_mouse <- rankrank_plot(mouse_rr, paste0("Mouse SVG rank–rank (top ", topN, ")"))
p_human <- rankrank_plot(human_rr, paste0("Human SVG rank–rank (top ", topN, ")"))
