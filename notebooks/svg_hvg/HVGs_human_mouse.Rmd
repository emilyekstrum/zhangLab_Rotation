...
```{r}
DefaultAssay(mouse_seurat) <- "SCT"
mouse_seurat <- RunPCA(mouse_seurat, verbose = FALSE) # first dim reduction via PCA
mouse_seurat <- FindNeighbors(mouse_seurat, dims = 1:30) # construct KNN graph
mouse_seurat <- FindClusters(mouse_seurat, resolution = 0.5) # cluster cells based on KNN graph (Louvain/Leiden algorithm)
```
...
# store mouse HVG in a gene by cell matrix
```{r mouse-hvg-matrix}
...
saveRDS(mouse_hvg_matrix, file = "/Users/emilyekstrum/repos/zhangLab_Rotation/data/processed/hvgs/mouse_hvg_gene_cell_matrix.rds")
```

# or store as mouse HVG cell by gene matrix
```{r}
mouse_hvg_matrix_gene_by_cell <- t(mouse_hvg_matrix)
dim(mouse_hvg_matrix_gene_by_cell) 

head(mouse_hvg_matrix_gene_by_cell[, 1:20])
```

# save mouse HVG cell by gene matrix
```{r save-mouse-hvg-matrix-gene-by-cell}
saveRDS(mouse_hvg_matrix_gene_by_cell, file = "/Users/emilyekstrum/repos/zhangLab_Rotation/data/processed/hvgs/mouse_hvg_cell_gene_matrix.rds")
```

# save gene by cell matrix as csv
```{r save-mouse-hvg-matrix-csv}
write.csv(mouse_hvg_matrix, file = "/Users/emilyekstrum/repos/zhangLab_Rotation/data/processed/hvgs/mouse_hvg_gene_cell_matrix.csv")
...
# store mouse HVG in a gene by cell matrix
```{r mouse-hvg-matrix}
human_hvg_matrix <- GetAssayData(human_seurat, assay = "SCT", layer = "data")[human_hvg, ]
human_hvg_matrix[1:5, 1:5]

head(rownames(human_hvg_matrix), 20)
```

# save human gene by cell HVG matrix
```{r save-hvg-matrix}
saveRDS(human_hvg_matrix, file = "/Users/emilyekstrum/repos/zhangLab_Rotation/data/processed/hvgs/human_hvg_gene_cell_matrix.rds")
```

# or store as mouse HVG cell by gene matrix
```{r}
human_hvg_matrix_cell_by_gene <- t(human_hvg_matrix)
dim(human_hvg_matrix_cell_by_gene) 

head(human_hvg_matrix_cell_by_gene[, 1:20])
```

# save mouse HVG gene by cell matrix
```{r}
saveRDS(human_hvg_matrix_cell_by_gene, file = "/Users/emilyekstrum/repos/zhangLab_Rotation/data/processed/hvgs/human_hvg_cell_gene_matrix.rds")
```

# save gene by cell matrix as csv
```{r}
write.csv(human_hvg_matrix, file = "/Users/emilyekstrum/repos/zhangLab_Rotation/data/processed/hvgs/human_hvg_gene_cell_matrix.csv")
